...
public class VoismartSwitchBridgeHandler extends BaseThingHandler {
    ...
    private Logger logger = LoggerFactory.getLogger(VoismartSwitchBridgeHandler.class);
	...
	public final static Set<ThingTypeUID> SUPPORTED_THING_TYPES = Collections
			.singleton(THING_TYPE_VOISMARTSWITCH);
			...
			try {
				try {
					...
					if (!lastBridgeConnectionState) {
						logger.debug(
								"Connection to VoismartSwitch {} established.",
								bridge.getIPAddress());
						lastBridgeConnectionState = true;
						onConnectionResumed(bridge);
					}
					if (lastBridgeConnectionState) {
						Map<String, FullPort> lastPortStateCopy = new HashMap<>(
								lastPortStates);
						...
						for (final FullPort fullPort : fullConfig.getPorts()) {
							...
							if (lastPortStateCopy.containsKey(portId)) {
								final FullPort lastFullPort = lastPortStateCopy
										.remove(portId);
								final OnOffType lastFullPortState = lastFullPort
										.getPOEStatus();
								lastPortStates.put(portId, fullPort);
								if (!isEqual(lastFullPortState,
										fullPort.getPOEStatus())) {
									logger.debug(
											"Status {} update for port {} detected.",
											fullPort.getPOEStatus(), portId);
									for (PortStatusListener portStatusListener : portStatusListeners) {
										try {
											logger.debug("Signalling port status changed");
											portStatusListener
													.onPortStateChanged(bridge,
															fullPort);
										} catch (Exception e) {
											logger.error(
								...
								for (PortStatusListener portStatusListener : portStatusListeners) {
									try {
										...
										portStatusListener.onPortAdded(bridge,
												fullPort);
									} catch (Exception e) {
										logger.error(
												"An exception occurred while calling the BridgeHeartbeatListener",
						...
						// mai)
						for (Entry<String, FullPort> fullPortEntry : lastPortStateCopy
								.entrySet()) {
							lastPortStates.remove(fullPortEntry.getKey());
							logger.debug("Port {} removed.",
									fullPortEntry.getKey());
							for (PortStatusListener portStatusListener : portStatusListeners) {
								try {
									portStatusListener.onPortRemoved(bridge,
											fullPortEntry.getValue());
								} catch (Exception e) {
									logger.error(
											"An exception occurred while calling the BridgeHeartbeatListener",
											e);
					...
					if (bridge != null) {
						if (lastBridgeConnectionState) {
							logger.debug("Connection to Hue Bridge {} lost.",
									bridge.getIPAddress());
							lastBridgeConnectionState = false;
							onConnectionLost(bridge);
						}
	...
	public void initialize() {
		...
		try {
			if (bridge == null) {
				...
				bridge = new VoismartSwitchBridge(
						InetAddress.getByName((String) config.get(HOST)),
						(String) config.get(USER_NAME),
						(String) config.get(USER_PASSWORD));
			}
			onUpdate();
		} catch (Exception ex) {
	...
	public void onNotAuthenticated(VoismartSwitchBridge bridge) {
		...
		if (userName != null && userPassword != null) {
			try {
				...
				logger.info(
						"Voismart bridge {} is not authenticated - please press the pairing button on the bridge.",
						getConfig().get(HOST));
				/*
	...
	public boolean unregisterPortStatusListener(
			PortStatusListener portStatusListener) {
